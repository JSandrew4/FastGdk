...
namespace Fast
{
	...
	Color::Color(Int red, Int green, Int blue, Int alpha) {
		mRed = Math::ClampFloat(((Float)red / 255.0f), 0.0f, 1.0f);
		mGreen = Math::ClampFloat(((Float)green / 255.0f), 0.0f, 1.0f);
		mBlue = Math::ClampFloat(((Float)blue / 255.0f), 0.0f, 1.0f);
		mAlpha = Math::ClampFloat(((Float)alpha / 255.0f), 0.0f, 1.0f);
	}

	Color& Color::operator=(const Color &that) {
		this->mRed = that.mRed;
		this->mGreen = that.mGreen;
		this->mBlue = that.mBlue;
		this->mAlpha = that.mAlpha;
		return * this;
	}

	Bool Color::operator==(const Color &that) const {
		if (!Math::IsSimilar(this->mRed, that.mRed, 0.001f)) return false;
		if (!Math::IsSimilar(this->mGreen, that.mGreen, 0.001f)) return false;
		if (!Math::IsSimilar(this->mBlue, that.mBlue, 0.001f)) return false;
		if (!Math::IsSimilar(this->mAlpha, that.mAlpha, 0.001f)) return false;
		return true;
	}

	Bool Color::operator!=(const Color &that) const {
		if (!Math::IsSimilar(this->mRed, that.mRed, 0.001f)) return true;
		if (!Math::IsSimilar(this->mGreen, that.mGreen, 0.001f)) return true;
		if (!Math::IsSimilar(this->mBlue, that.mBlue, 0.001f)) return true;
		if (!Math::IsSimilar(this->mAlpha, that.mAlpha, 0.001f)) return true;
		return false;
	}
	...
	const Color Color::Gainsbobro			= Color(220, 220, 220, 255);
